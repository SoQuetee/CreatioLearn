define("BaseViewModule", ["BaseViewModuleResources", "performancecountermanager", "ConfigurationConstants",
	...
	Ext.define("Terrasoft.configuration.BaseViewModule", {
		...
		getHomePagePath: function() {
			...
			if (Terrasoft.Features.getIsDisabled("DisableHomePageInWorkplace") && !this._useCurrentUserHomePage) {
				const workplace = HomePageInWorkplaceUtils.getCurrentWorkplace(workplaceId);
				if (workplace?.homePageSchemaName &&
					...
					return Terrasoft.combinePath("HomePage", workplace.homePageSchemaName);
				}
				if (workplace?.homePageSchemaName) {
					return Terrasoft.combinePath("IntroPage", workplace.homePageSchemaName);
				} else {
					_homeModule = this.getDefaultWorkplaceModule();
				}
			}
			const module = Terrasoft.configuration.ModuleStructure[_homeModule];
			return module

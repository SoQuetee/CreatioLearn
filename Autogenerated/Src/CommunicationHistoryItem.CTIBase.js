			...
			return {
				...
				methods: {
					...
					getSubscriberUrl: function() {
						...
						var subscriberId = this.get("SubscriberId");
						if (!this.Ext.isEmpty(schemaName)) {
							const linkHrefTpl = "{0}?schemaName={1}&recordId={2}";
							var loaderPath = Terrasoft.loaderBaseUrl;
							if (!Ext.String.endsWith(loaderPath, "/")) {
								loaderPath = loaderPath.concat("/");
							}
							const requestUrl = loaderPath.concat(this.$NavigationPath);
							const linkHref = Ext.String.format(linkHrefTpl, requestUrl, schemaName, subscriberId);
							return linkHref;
						} else {
							return "";
						}

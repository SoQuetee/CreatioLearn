namespace Terrasoft.Configuration
{
	...
	public static class ActivityUtils {
		...
		public static string GetActivityHash(string title, string location, DateTime startDate, DateTime dueDate, Guid priorityId, string notes, TimeZoneInfo timeZoneInfo) {
			DateTime utcStartDate = new DateTime(startDate.Ticks, DateTimeKind.Unspecified);
			utcStartDate = TimeZoneInfo.ConvertTimeToUtc(utcStartDate, timeZoneInfo);
			DateTime utcDueDate = new DateTime(dueDate.Ticks, DateTimeKind.Unspecified);
			utcDueDate = TimeZoneInfo.ConvertTimeToUtc(utcDueDate, timeZoneInfo);
			return FileUtilities.GetMD5Hash(Encoding.Unicode.GetBytes(title + location + utcStartDate.ToString() + utcDueDate.ToString() + priorityId.ToString() + notes));
		}
		...
		public static string GetSenderEmail(Entity entity) {
			...
			if (senderColumn != null) {
				...
				return sender.ParseEmailAddress()[0];
			} 
			return string.Empty;
		}

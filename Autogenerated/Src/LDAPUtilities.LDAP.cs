namespace Terrasoft.Configuration.LDAP
{
	...
	public class LdapUtilities : IDisposable
	{
		...
		public LdapUtilities(UserConnection userConnection) {
			...
			_ldapServer = SysSettings.GetValue(userConnection, LdapServerSysSettingCode).ToString();
			_ldapServerLogin = SysSettings.GetValue(userConnection, LdapServerLoginSysSettingCode).ToString();
			_ldapServerPassword = SysSettings.GetValue(userConnection, LdapServerPasswordSysSettingCode).ToString();
			try {
		...
		private static bool VerifyServerCertificate(LdapConnection connection, X509Certificate certificate) {
			...
			try {
				...
				if(_log.IsDebugEnabled) {
					List<string> chainErrors = GetCertificateChainErrors(certificate2);
					if(chainErrors.Any()) {
						var chainError = string.Join("\n", chainErrors);
						_log.Debug($"Chain errors were detected during certificate validation:{chainError}");
					}
				}
				return false;
			}

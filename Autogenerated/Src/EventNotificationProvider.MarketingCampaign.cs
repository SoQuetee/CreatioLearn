namespace Terrasoft.Configuration
{
	...
	#region Class: EventNotificationProvider

	/// <summary>
	/// Event notification provider.
	/// </summary>
	public class EventNotificationProvider : BaseNotificationProvider, INotificationProvider
	{
		...
		public EventNotificationProvider(UserConnection userConnection)
			: base(userConnection) {
		}

		public EventNotificationProvider(Dictionary<string, object> parameters)
			: base(parameters) {
		}
		...
		protected override void AddColumns(Select select) {
			select
				...
				.Column("Event", "Id").As("EventId")
				.Column("Event", "Name").As("Title")
		...
		protected override void JoinTables(Select select) {
			select
				...
				.InnerJoin("Event").On("Reminding", "SubjectId").IsEqual("Event", "Id")
		...
		protected override INotificationInfo GetRecordNotificationInfo(
			...
			return new NotificationInfo() {
				...
				ImageId = imageId,
				EntityId = new Guid(dictionaryColumnValues["EventId"]),
		...
		public override string GetRecordResult(Dictionary<string, string> dictionaryColumnValues) {
			...
			var key = id + "_" + remindTime;var popup = new PopupData() {
				Title = title,
				Body = body,
				EntityId = subjectId,
